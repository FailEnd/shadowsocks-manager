...
const checkOrder = async paypalId => {
  ...
  await knex('paypal').update({
    status: orderInfo.state,
    paypalData: JSON.stringify(orderInfo)
  }).where({
    paypalId
  });
  return;
...
const orderList = async (options = {}) => {
  ...
  const orders = await knex('paypal').select(['paypal.orderId', 'paypal.orderType', 'user.id as userId', 'user.username', 'account_plugin.port', 'paypal.amount', 'paypal.status', 'paypal.paypalData', 'paypal.createTime', 'paypal.expireTime']).leftJoin('user', 'user.id', 'paypal.user').leftJoin('account_plugin', 'account_plugin.id', 'paypal.account').where(where).orderBy('paypal.createTime', 'DESC');
  orders.forEach(f => {
...
const orderListAndPaging = async (options = {}) => {
  ...
  if (group >= 0) {
    count = count.leftJoin('user', 'user.id', 'paypal.user').where({
      'user.group': group
    });
    orders = orders.where({
      'user.group': group
...
const getUserFinishOrder = async userId => {
  let orders = await knex('paypal').select(['orderId', 'amount', 'createTime']).where({
    user: userId
...
cron.minute(() => {
  ...
  knex('paypal').delete().where({
    status: 'created'
  }).whereBetween('expireTime', [0, Date.now() - 1 * 24 * 3600 * 1000]).then();
}, 30);