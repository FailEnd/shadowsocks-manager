...
exports.addAccount = (req, res) => {
  req.checkBody('port', 'Invalid port').isInt({
    min: 1,
    max: 65535
  });
...
exports.deleteUserAccount = (req, res) => {
  ...
  macAccount.getAccountByAccountId(accountId).then(macAccounts => {
    ...
    return account.editAccount(accountId, {
      userId: null
    });
  }).then(success => {
    res.send(success);
...
exports.getCsvOrders = async (req, res) => {
  ...
  options.end = req.query.end;
  options.filter = req.query.filter || '';
  alipay.getCsvOrder(options).then(success => {
...
exports.newPortForAddAccount = async (req, res) => {
  try {
    ...
    if (port.random) {
      ...
      newPort = await knex('account_plugin').select().whereBetween('port', [port.start, port.end]).orderBy('port', 'ASC').then(success => {
