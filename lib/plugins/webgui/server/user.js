...
exports.getMultiServerFlowStatus = (req, res) => {
  knex('webguiSetting').select().where({
    ...
    return res.send({
      status: success.value.multiServerFlow
    });
  }).catch(err => {
    console.log(err);
    res.status(403).end();
...
exports.payByGiftCard = async (req, res) => {
  ...
  try {
    const checkGiftcardType = async () => {
      ...
      const giftCardInfo = await knex('giftcard').where({
        password
      }).then(s => s[0]);
      const validTypes = currentOrder.length ? currentOrder.map(m => m.id) : orders.map(m => m.id);
      return validTypes.indexOf(giftCardInfo.orderType) < 0; // if(!accountId) { return false; }
...
exports.getAccountSubscribe = async (req, res) => {
  try {
    const userId = req.session.user;
    const accountId = +req.params.accountId;
    const account = await knex('account_plugin').select(['id', 'subscribe']).where({
      id: accountId,
      userId
    }).then(s => s[0]);
