...
exports.searchRefUser = async (req, res) => {
  try {
    const search = req.body.search;
    const users = await knex('user').select(['user.id as id', 'user.username as username']).leftJoin('webgui_ref', 'webgui_ref.userId', 'user.id').where('user.username', 'like', `%${search}%`).where({
      'user.type': 'normal'
