...
exports.signup = (req, res) => {
  ...
  req.getValidationResult().then(result => {
    ...
    if (success[0] > 1) {
      const userId = success[0]; // let port = 50000;
    ...
    logger.error(`[${req.body.email}] signup fail: ${err}`); // res.status(403).end();
...
exports.status = async (req, res) => {
  ...
  try {
    ...
    if (status) {
      email = (await knex('user').select(['email']).where({
        id
      }).then(s => s[0])).email;
    ...
    if (status === 'normal') {
      ...
      const groupId = (await knex('user').select(['group']).where({
        id
      }).then(s => s[0])).group;
      multiAccount = (await knex('group').where({
        id: groupId
      }).then(s => s[0])).multiAccount;
    }
