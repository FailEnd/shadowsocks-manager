...
const visitRefCode = async code => {
  ...
  const sourceUserInfo = (await knex('user').where({
    id: codeInfo.sourceUserId
  }))[0];
  ...
  const currentRefUser = await knex('webgui_ref').where({
    codeId: codeInfo.id
  });
  ...
  await knex('webgui_ref_code').where({
    code
  }).increment('visit', 1);
  return true;
};
