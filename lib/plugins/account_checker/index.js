...
const isPortExists = async (server, account) => {
  const ports = (await manager.send({
    command: 'list'
  }, {
    host: server.host,
    port: server.port,
  ...
  if (account.type >= 2 && account.type <= 5) {
    ...
    if (expireTime <= Date.now() || data.create >= Date.now()) {
      ...
      if (account.active && account.autoRemove && expireTime + account.autoRemoveDelay < Date.now()) {
        ...
        knex('account_plugin').delete().where({
          id: account.id
        }).then();
      } else {
...
const isBaned = async (server, account) => {
  ...
  if (!accountFlowInfo.autobanTime || Date.now() > accountFlowInfo.autobanTime) {
    await knex('account_flow').update({
      ...
      id: accountFlowInfo.id
    });
    return false;
  }

  await knex('account_flow').update({
    nextCheckTime: accountFlowInfo.autobanTime
  }).where({
    id: accountFlowInfo.id
  });
  return true;
...
const deleteExtraPorts = async serverInfo => {
  try {
    const currentPorts = await manager.send({
      command: 'list'
    }, {
...
const checkAccount = async (serverId, accountId) => {
  try {
    ...
    const exists = await isPortExists(serverInfo, accountInfo); // 检查账号是否激活
...
(async () => {
  while (true) {
    ...
    try {
      ...
      await sleep(sleepTime);
      const accounts = await knex('account_plugin').select(['account_plugin.id as id']).crossJoin('server').leftJoin('account_flow', function () {
...
(async () => {
  while (true) {
    ...
    try {
      const datas = await knex('account_flow').select().where('nextCheckTime', '<', Date.now()).orderBy('nextCheckTime', 'asc').limit(600);
      accounts = [...accounts, ...datas];

      if (datas.length < 30) {
        accounts = [...accounts, ...(await knex('account_flow').select().where('nextCheckTime', '>', Date.now()).orderBy('nextCheckTime', 'asc').limit(30 - datas.length))];
      }
    } catch (err) {
      console.log(err);
    }
    ...
    try {
      const datas = await knex('account_flow').select().orderBy('updateTime', 'desc').where('checkTime', '<', Date.now() - 60000).limit(15);
      accounts = [...accounts, ...datas];
    } catch (err) {
      console.log(err);
    }
    ...
    try {
      datas = await knex('account_flow').select().orderByRaw('rand()').limit(5);
      accounts = [...accounts, ...datas];
    } catch (err) {}

    try {
      datas = await knex('account_flow').select().orderByRaw('random()').limit(5);
      accounts = [...accounts, ...datas];
    } catch (err) {}
