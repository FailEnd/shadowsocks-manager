...
const changePort = async (id, port) => {
  const result = await knex('account_plugin').update({
    port
  }).where({
    id
  });
...
const getAccount = async (options = {}) => {
  ...
  const account = await knex('account_plugin').select(['account_plugin.id', 'account_plugin.type', 'account_plugin.orderId', 'account_plugin.userId', 'account_plugin.server', 'account_plugin.port', 'account_plugin.password', 'account_plugin.data', 'account_plugin.status', 'account_plugin.autoRemove', 'account_plugin.autoRemoveDelay', 'account_plugin.multiServerFlow', 'account_plugin.active', 'user.id as userId', 'user.email as user']).leftJoin('user', 'user.id', 'account_plugin.userId').where(where);
  return account;
};
...
const delAccount = async id => {
  ...
  const result = await knex('account_plugin').delete().where({
    id
  });
...
const editAccount = async (id, options) => {
  ...
  await knex('account_plugin').update(update).where({
    id
  });
...
const editAccountTime = async (id, timeString, check) => {
  ...
  let accountInfo = await knex('account_plugin').where({
    id
  }).then(s => s[0]);
...
const changePassword = async (id, password) => {
  const account = await knex('account_plugin').select().where({
    id
  }).then(success => {
    if (success.length) {
      return success[0];
...
const addAccountLimitToMonth = async (userId, accountId, number = 1) => {
  if (!accountId) {
    const port = await knex('account_plugin').select().orderBy('port', 'DESC').limit(1).then(success => {
      if (success.length) {
...
const setAccountLimit = async (userId, accountId, orderId) => {
  ...
  if (accountId) {
    account = await knex('account_plugin').select().where({
      id: accountId
    }).then(success => {
      if (success.length) {
        return success[0];
...
const getBanAccount = async options => {
  const serverId = options.serverId;
  const accountId = options.accountId;
  const accountInfo = await knex('account_flow').select(['autobanTime as banTime']).where({
    serverId,
    accountId,
    status: 'ban'
  });
...
const editMultiAccounts = async (orderId, update) => {
  ...
  for (const account of accounts) {
    ...
    await knex('account_plugin').update(updateData).where({
      id: account.id
    });
