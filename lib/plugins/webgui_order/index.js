...
const getOneOrderByAccountId = async accountId => {
  const order = await knex('webgui_order').select(['webgui_order.id as id', 'webgui_order.changeOrderType as changeOrderType']).leftJoin('account_plugin', 'account_plugin.orderId', 'webgui_order.id').where({
...
const deleteOrder = async orderId => {
  const hasAccount = await knex('account_plugin').where({
    orderId
  });
  ...
  const hasGiftcard = isGiftCardOn ? await knex('giftcard').where({
    orderType: orderId,
    status: 'AVAILABLE'
  }) : [];
  ...
  await knex('webgui_order').delete().where({
    id: orderId
  });
  return;
};
