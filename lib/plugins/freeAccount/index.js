...
const checkPort = async () => {
  ...
  const accounts = await manager.send({
    command: 'list'
  });
  ...
  if (!exists) {
    ...
    const createTime = (await getKey('create', {
      time: Date.now()
    })).time;
    ...
    const currentFlow = (await getKey('flow', {
      flow: 0
    })).flow;
    ...
    const sumFlow = (await getKey('sumFlow', {
      flow: 0
    })).flow;
    await setKey('sumFlow', {
      flow: sumFlow + newFlow
    });
    logger.info('sumFlow: ' + prettyFlow(sumFlow + newFlow));
    const ips = await manager.send({
      command: 'ip',
      port: currentPort
    });
    logger.info(ips);
