...
cron.minute(async () => {
  ...
  const scanOrder = order => {
    ...
    if (order.status !== 'TRADE_SUCCESS' && order.status !== 'FINISH') {
      ...
      return account.setAccountLimit(userId, accountId, order.orderType).then(() => {
        return knex('alipay').update({
          status: 'FINISH'
        }).where({
          orderId: order.orderId
...
const orderListAndPaging = async (options = {}) => {
  ...
  if (group >= 0) {
    count = count.leftJoin('user', 'user.id', 'alipay.user').where({
      'user.group': group
    });
    orders = orders.where({
      'user.group': group
...
const getUserFinishOrder = async userId => {
  let orders = await knex('alipay').select(['orderId', 'amount', 'createTime']).where({
    user: userId
...
cron.minute(() => {
  ...
  knex('alipay').delete().where({
    status: 'CREATE'
  }).whereBetween('createTime', [0, Date.now() - 1 * 24 * 3600 * 1000]).then();
}, 50);
