...
const getFlow = function () {
  if (arguments[3]) {
    ...
    return knex('saveFlow').sum('flow as sumFlow').groupBy('port').select(['port']).where({
      id
    }).whereBetween('time', [startTime, endTime]);
  } else {
...
const child = appFork('plugins/flowSaver/flowChildProcess'); // child.setMaxListeners(200);
...
const getFlowFromSplitTime = async (serverId, accountId, start, end) => {
  ...
  if (serverId) {
    ...
    getFlow = (tableName, startTime, endTime) => {
      return knex(tableName).sum('flow as sumFlow').groupBy('id').select(['id']).where(where).whereBetween('time', [startTime, endTime - 1]).then(success => {
        if (success[0]) {
...
const getFlowFromSplitTimeWithScale = async (serverIds, accountId, start, end) => {
  ...
  const getFlow = (tableName, startTime, endTime) => {
    ...
    const where = {};
    where[`${tableName}.accountId`] = accountId;
    return knex('server').sum(`${tableName}.flow as sumFlow`).groupBy(['server.id', `${tableName}.accountId`]).select(['server.id', `${tableName}.accountId`, 'server.scale']).leftJoin(tableName, `${tableName}.id`, 'server.id').whereBetween(`${tableName}.time`, [startTime, endTime - 1]).whereIn('server.id', serverIds).andWhere(where);
  };
...
const getServerPortFlowWithScale = async (serverId, accountId, timeArray, isMultiServerFlow) => {
  ...
  return [sumFlow]; // const getOneServerFlow = async (serverId, accountId, timeArray) => {
  //   const result = [];
  //   timeArray.forEach((time, index) => {
  //     if(index === timeArray.length - 1) {
  //       return;
...
const getlastConnectTime = async (serverId, accountId) => {
  const lastConnectFromSaveFlow = await knex('saveFlow').select(['time']).where({
    id: serverId,
    accountId
  ...
  if (lastConnectFromSaveFlow) {
    return {
      lastConnect: lastConnectFromSaveFlow
    };
  }

  return knex('saveFlow5min').select(['time']).where({
    id: serverId,
    accountId
  }).orderBy('time', 'desc').limit(1).then(success => {
    if (success[0]) {
      return {
        lastConnect: success[0].time
      };
    }
...
const getUserPortLastConnect = async accountId => {
  ...
  let knexQuery = knex('saveFlow').select(['time']).where({
    accountId
  });
  let knex5MinQuery = knex('saveFlow5min').select(['time']).where({
    accountId
  });
  const lastConnectFromSaveFlow = await knexQuery.orderBy('time', 'desc').limit(1).then(success => {
    if (success[0]) {
      return success[0].time;
    }
  ...
  return knex5MinQuery.orderBy('time', 'desc').limit(1).then(success => {
    if (success[0]) {
      return {
        lastConnect: success[0].time
      };
...
const getAccountServerFlow = (accountId, timeArray) => {
  ...
  return knex(tableName).sum(`${tableName}.flow as flow`).groupBy(`${tableName}.id`).select(['server.name']).leftJoin('server', 'server.id', `${tableName}.id`).leftJoin('account_plugin', 'account_plugin.id', `${tableName}.accountId`).where({
    'account_plugin.id': accountId
...
const getTopFlow = groupId => {
  ...
  return knex('saveFlow').sum('saveFlow.flow as sumFlow').groupBy('user.id').orderBy('sumFlow', 'desc').limit(5).select(['user.id as userId', 'user.username as email', 'account_plugin.port as port', 'account_plugin.id as accountId']).leftJoin('account_plugin', 'account_plugin.id', 'saveFlow.accountId').innerJoin('user', 'account_plugin.userId', 'user.id').whereBetween('saveFlow.time', [startTime, endTime]).where(where);
};
...
const cleanAccountFlow = async id => {
  ...
  await knex('saveFlow').update({
    accountId: 0
  }).where({
    accountId: id
  });
  await knex('saveFlow5min').update({
    accountId: 0
  }).where({
    accountId: id
  });
  await knex('saveFlowHour').update({
    accountId: 0
  }).where({
    accountId: id
  });
  await knex('saveFlowDay').update({
    accountId: 0
  }).where({
    accountId: id
  });
