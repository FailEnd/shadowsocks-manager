...
const getAccount = async (userId, group) => {
  ...
  const macAccounts = await knex('mac_account').select(['mac_account.id as id', 'mac_account.userId as userId', 'mac_account.mac as mac', 'mac_account.accountId as accountId', 'mac_account.serverId as serverId']).where(where).leftJoin('user', 'user.id', 'mac_account.userId');
...
const getNoticeForUser = async (mac, ip) => {
  ...
  const macAccount = await knex('mac_account').where({
    mac
  ...
  const groupInfo = await knex('user').select(['group.id as id', 'group.showNotice as showNotice']).innerJoin('group', 'user.group', 'group.id').where({
    'user.id': userId
  }).then(s => s[0]);
...
const getAccountForUser = async (mac, ip, opt) => {
  ...
  const serverList = servers.filter(f => {
    ...
    return getIp(f.host).then(success => {
      ...
      if (startTime && !noFlow) {
        return getFlow(isMultiServerFlow ? null : success.id, account.accountId); // return flow.getFlowFromSplitTime(isMultiServerFlow ? null : success.id, account.accountId, startTime, Date.now());
      } else {
        return -1;
      }
    }).then(success => {
...
const deleteAccount = id => {
  return knex('mac_account').delete().where({
    id
  }).then();
};
...
const removeInvalidMacAccount = async () => {
  const accounts = await knex('mac_account').select(['mac_account.id as id', 'mac_account.mac as mac', 'mac_account.userId as userId', 'mac_account.accountId as accountId', 'mac_account.serverId as serverId', 'user.username as username']).leftJoin('user', 'mac_account.userId', 'user.id').where({});
  accounts.filter(f => {
    return f.username === null;
  }).forEach(account => {
    knex('mac_account').where({
      id: account.id
    }).del().then();
  });
};
...
const userAddMacAccount = async (userId, mac) => {
  ...
  const currentMacAccount = await knex('mac_account').where({
    userId
  });
  const insertData = {
    mac: macAddress,
  ...
  const userAccount = await knex('account_plugin').where({
    userId
  });
