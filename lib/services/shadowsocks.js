...
const sendMessage = message => {
  const randomTraceNumber = Math.random().toString().substr(2, 6); // logger.info(`[${ randomTraceNumber }] Send to shadowsocks: ${ message }`);
...
const changePassword = async (port, password) => {
  try {
    const updateAccount = await knex('account').where({
      port
    }).update({
      password
    });
...
const getFlow = async options => {
  try {
    ...
    const flows = await knex('flow').select(['port']).sum('flow as sumFlow').groupBy('port').whereBetween('time', [startTime, endTime]);
    accounts.map(m => {
